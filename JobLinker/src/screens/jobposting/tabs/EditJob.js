...
const EditJob = () => {
  ...
  const validate = () => {
    ...
    if (jobDesc == '') {
      ...
      validJobDesc = false;
      setBadJobDecs('Lütfen açıklama çin min 50 karakter girin!');
    } else if (jobDesc != '' && jobDesc.length >= 50) {
      validJobDesc = true;
      setBadJobDecs('');
    }
    ...
    } else if (exp != '' && !exp.match(expRegex)) {
      validExp = false;
      setBadExp('Lütfen geçerli bir deneyim giriniz!');
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <CustomTextInput
        ...
        title={'İş Açıklaması'}
        placeholder={'ör. bu bir web geliştirme işi'}
